...
def validate_digits_input(message, values_range=None):
    ...
    input_number = None
    while True:
        brain.output_engine.assistant_response(message)
        ...
        if values_range:
            ...
            if not min_value <= input_number <= max_value:
                brain.output_engine.assistant_response\
...
def check_input_to_continue(message=''):
    ...
    return brain.input_engine.recognize_input(already_activated=True) in positive_answers


def validate_input_with_choices(available_choices):
    user_input = brain.input_engine.recognize_input(already_activated=True)
    while not user_input in available_choices:
        brain.output_engine.assistant_response('Please select on of the values: {0}'.format(available_choices))
        user_input = brain.input_engine.recognize_input(already_activated=True)
    return user_input
